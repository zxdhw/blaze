...
RESULT=$HOME/blaze/analysis/graph/time/H5300/hit/D64
...
TIMES=1
qd=1
declare -a hitSize=("2" "4" "8")

# declare -a hitSize=("1" "2" "4" "8" "16" "32" "64" "96" "127")
declare -a apps=("bfs")
#data and index
...
#             k="${index[i]}"
#             j="${data[i]}"
#             sudo $APP_PATH/${e} -computeWorkers $COMPUTEWORKERS -startNode $STARTNODE -queueDepth $qd -hit $HIT $INDEX/${k} $DATA/${j} > breakdown_nomerge_qd128.out
    ...
    for h in "${hitSize[@]}"; do
        for e in "${apps[@]}"; do
            for ((i=0; i<${#index[@]}; i++)); do
                k="${index[i]}"
                j="${data[i]}"
                sudo $APP_PATH/${e} -computeWorkers $COMPUTEWORKERS -startNode $STARTNODE -hit $HIT -queueDepth $qd -hitSize ${h} $INDEX/${k} $DATA/${j} >> breakdown_hit${h}_qd1.out
