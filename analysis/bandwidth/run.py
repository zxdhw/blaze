...
BLAZE_BINARY_PATH = "/home/zhengxd/blaze/build/bin"
...
def build_command(args, data_format):
    ...
    # hitchhike
    cmd += ' ' + '-hit=' + str(args.hit)
    cmd += ' ' + '-hitSize=' + str(args.hitSize)
    cmd += ' ' + '-queueDepth=' + str(args.queueDepth)
...
if __name__ == "__main__":
    ...
    #For hit
    parser.add_argument('--hit', default=0, type=int,
                      help='enable hit ?')
    parser.add_argument('--hitSize', default=64, type=int,
                      help='the number of hitchhiker')
    parser.add_argument('--queueDepth', default=256, type=int,
                      help='libaio queue septh')
