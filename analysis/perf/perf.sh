...
PERF_PARSED_LIST='/home/zxd/blaze/analysis/perf/dapuH'
PERF_PARSED_GRAPH='/home/zxd/blaze/analysis/perf/dapuH'
PERF_RES='/home/zxd/blaze/analysis/perf/dapuH'
...
perf record -C $CORE -e instructions -F 99 -o $PERF_RES/${EBPF}_${e}.perf.out $APP_PATH/${e} -computeWorkers $THREAD -startNode $STARTNODE -ebpf $EBPF $INDEX $DATA;
...
for e in "${apps[@]}"
do
perf report --vmlinux $VMLINUX -n -m --stdio --full-source-path --source -s symbol -i $PERF_RES/${EBPF}_${e}.perf.out >> $PERF_PARSED_LIST/${EBPF}_perf_parsed_${e}.txt;
